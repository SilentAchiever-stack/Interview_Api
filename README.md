...
A backend API for an interview question bank. Users can register, log in, and browse
interview questions filtered by topic or difficulty, while admin accounts can add,
update, and remove questions. Built with Node.js, Express, and MongoDB, and secured
...
platform. A frontend (web app, mobile app, or a tool like Postman) can connect to it so
users can create an account, verify their email, log in, and then browse a bank of
interview questions. Regular users have read-only access to the question bank. Users
with an "admin" role can create, edit, replace, and delete questions, keeping the
...
- Only accounts with an "admin" role can add, update, replace, or delete questions.
  This is enforced through dedicated middleware that checks the role stored in the
  user's JWT.

**Rate Limiting**
- Every route is protected by a rate limiter to reduce the risk of abuse or
  brute-force attempts, particularly on login and OTP-related endpoints
...
Requests pass through a chain of middleware before reaching their controller. Adding a
new question, for example, passes through the rate limiter, then an auth check (valid
JWT?), then an admin check (is this user an admin?), and only then the actual
controller logic. This keeps permission logic separate from business logic, so each
layer is easier to test and reason about on its own.
...
**Role-based permissions as a distinct layer.** Instead of checking "is this user an
admin" inside every controller function, I pulled that check out into its own reusable
middleware that runs before the controller. This keeps the authorization logic in one
place and makes it trivial to add or remove admin protection from any route.
...
logic instead of mixing validation and data-handling concerns together.

**Disabling incomplete features safely.** Some features, like Google OAuth and Stripe
payments, were scaffolded early but aren't fully wired up yet. Rather than deleting
that code, I commented out the relevant requires and routes in server.js. This taught
me an important lesson: commenting out a file's usage isn't enough on its own if the
file is also excluded from version control. Both the file and every place that imports
it need to be in sync, or the app will crash on deployment with a "module not found"
error even though it runs fine locally.

**Deploying and debugging in a live environment.** Errors that never show up locally
can surface only after deployment, like a missing dependency that was installed
locally but never added to package.json, or a required file that was excluded via
.gitignore but still referenced elsewhere in the code. Reading deployment logs
carefully, one line at a time, was essential to tracing these back to their actual
cause.
...
- Finish and enable Stripe-based payments and webhook handling
